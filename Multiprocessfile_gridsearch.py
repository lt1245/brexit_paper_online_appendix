...
def LoopFunc(x,gamma,Skill_level_no,Beta,degree,No_countries,coeff_e,liquid_param_asset,illiquid_param_asset,liquid_lower_bound,liquid_upper_bound,
             ...
             knots_liquid,knots_illiquid,Cons_NOadj,Cons_adj,Phi_fine,AssetMultiplied,s_fine,fine_grid_no,illiquid_param_asset_fine, Profits,Czship):
    #def util(cons):
...
#    else:
    ...
    #Non-adjust
    #real_income = (Wage[co1] * Skill_level1[skill_it,co1] +price[0]*np.minimum(s[:,1]/Bond,liquid_upper_bound) + price[co2]*Cap_adj_lim)/(price[co1])       
    real_income = (Wage[co1] * Skill_level1[skill_it,co1] +price[0]*s[:,1]/Bond + price[co2]*Cap_adj_lim+ Profits[co2] / Czship[co2])/(price[co1])      
